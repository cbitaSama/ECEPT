...
// Strip emojis y caracteres Unicode no-Latin que jsPDF no puede renderizar
// con la fuente Helvetica default (default sin embed Unicode).
function CB_stripEmojisForPDF(text) {
  if (!text) return '';
  return String(text)
...
function CB_parseMdTableForPDF(tableLines) {
  if (tableLines.length < 2) return null;
  function splitRow(line) {
    var s = line.trim();
    if (s.charAt(0) === '|') s = s.slice(1);
    if (s.charAt(s.length - 1) === '|') s = s.slice(0, -1);
    return s.split('|').map(function(c) { return CB_stripMdPlain(CB_stripEmojisForPDF(c.trim())); });
  }
  var headers = splitRow(tableLines[0]);
  var rows = [];
  for (var j = 2; j < tableLines.length; j++) {
    var cells = splitRow(tableLines[j]);
...
function CB_doExportToPDF(content, meta) {
  ...
  function addText(text, opts) {
    ...
    pageBreakIfNeeded(lines.length * (fontSize * 0.42));
    doc.text(lines, margin + indent, y);
    y += lines.length * (fontSize * 0.42) + (opts.spacingAfter || 4);
  }
  ...
  doc.text(CB_stripEmojisForPDF(meta.title || 'Documento ECEPT'), margin, y, { maxWidth: contentW });
  y += 12;
  // Línea decorativa
  ...
  while (i < lines.length) {
    ...
    // Skip horizontal rules + vacíos
    if (trimmed === '---' || trimmed === '***' || trimmed === '___') { y += 4; i++; continue; }
    ...
    if (trimmed.indexOf('|') === 0 && i + 1 < lines.length &&
      ...
      var td = CB_parseMdTableForPDF(tableLines);
      if (td && doc.autoTable) {
        ...
        doc.autoTable({
          ...
          styles: { fontSize: 9, cellPadding: 3, textColor: [30, 30, 30], lineColor: [200, 200, 220], lineWidth: 0.3 },
          headStyles: { fillColor: [40, 60, 130], textColor: [220, 230, 255], fontStyle: 'bold', fontSize: 9 },
          alternateRowStyles: { fillColor: [245, 247, 255] },
          theme: 'grid'
        });
        y = (doc.lastAutoTable && doc.lastAutoTable.finalY ? doc.lastAutoTable.finalY : y + 30) + 8;
      } else if (td) {
        // Fallback: tabla como texto cuando autoTable no cargó
        addText('[Tabla] ' + td.headers.join(' | '), { fontStyle: 'bold', fontSize: 10, spacingAfter: 2 });
        td.rows.forEach(function(r) { addText('  ' + r.join(' | '), { fontSize: 10, spacingAfter: 1 }); });
        y += 4;
      }
      continue;
    }
    ...
    if (/^\s*```/.test(line)) {
      ...
      codeLines.forEach(function(cl) {
        addText(CB_stripEmojisForPDF(cl), { fontSize: 9, color: [100, 100, 120], spacingAfter: 1 });
      });
      y += 4;
      continue;
    }
    ...
    if (hMatch) {
      ...
      var hText = CB_stripMdPlain(CB_stripEmojisForPDF(hMatch[2]));
      var hSizes = [16, 14, 12, 11];
      y += 4;
      addText(hText, {
        fontSize: hSizes[hLevel - 1] || 11,
        fontStyle: 'bold',
    ...
    if (bMatch) {
      addText('• ' + CB_stripMdPlain(CB_stripEmojisForPDF(bMatch[1])), { indent: 4, spacingAfter: 2 });
      i++; continue;
    }
    ...
    if (nMatch) {
      addText(nMatch[1] + '. ' + CB_stripMdPlain(CB_stripEmojisForPDF(nMatch[2])), { indent: 4, spacingAfter: 2 });
      i++; continue;
    }
  ...
  for (var p = 1; p <= totalPages; p++) {
    ...
    doc.text('Generado por Elion · ECEPT · ' + new Date().toLocaleDateString('es'), margin, 290);
    doc.text('Pagina ' + p + ' de ' + totalPages, pageW - margin - 25, 290);
  }

...
function DzGrid(p){
  var c=p.c||C.anx;
  return e("div",null,
    e("div",{style:{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))",gap:14,margin:"14px 0 10px"}},
      p.items.map(function(it,i){
        return e("button",{key:i,onClick:function(){if(p&&p.onOpen)p.onOpen(it);},style:{padding:"18px 18px 16px",background:"linear-gradient(180deg,rgba(13,18,36,0.7),rgba(10,14,31,0.5))",border:"1px solid "+ax(c,.20),borderRadius:14,cursor:"pointer",textAlign:"left",display:"flex",flexDirection:"column",gap:6,minHeight:130,transition:"transform 220ms cubic-bezier(0.16,1,0.3,1),border-color 220ms ease-out,box-shadow 220ms ease-out",fontFamily:"inherit",animation:"fadeIn .3s ease-out "+(i*40)+"ms both"},
          onMouseEnter:function(ev){ev.currentTarget.style.transform="translateY(-2px)";ev.currentTarget.style.borderColor=ax(c,.50);ev.currentTarget.style.boxShadow="0 8px 24px rgba(0,0,0,0.25)";},
          onMouseLeave:function(ev){ev.currentTarget.style.transform="translateY(0)";ev.currentTarget.style.borderColor=ax(c,.20);ev.currentTarget.style.boxShadow="none";}},

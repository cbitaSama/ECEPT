...
// Grid de tarjetas de enfermedades. Al host (SM App) le pasamos onOpen, y al
// tocar una tarjeta delegamos — la App promueve la enfermedad a ruta de
// primer nivel (view="disease") y la vista del hub es reemplazada por
// DzDetail. Esto replica el patrón reuma_dis de ECEPT: una sola vista
// activa a la vez, sin acordeón ni modal anidado, sin botón "Cerrar".
// Si onOpen no está presente (mount standalone del artefacto original),
// caemos al comportamiento legacy inline con DzModal como fallback.
function DzGrid(p){
  var c=p.c||C.anx;
  var s1=useState(null);var open=s1[0],setOpen=s1[1];
  var hasHost=!!p.onOpen;
  if(!hasHost && open!==null){
    var it=p.items[open];
  ...
  return e("div",null,
    e("div",{style:{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:10,margin:"14px 0 10px"}},
      p.items.map(function(it,i){
        return e("button",{key:i,onClick:function(){if(hasHost){p.onOpen(it);}else{setOpen(i);}},style:{padding:"14px 12px",background:"linear-gradient(135deg,"+ax(c,.15)+","+ax(c,.04)+" 85%)",border:"1px solid "+ax(c,.35),borderRadius:12,cursor:"pointer",textAlign:"left",display:"flex",flexDirection:"column",gap:4,minHeight:110,transition:"transform .15s"}},
  ...
  return e("div",{style:{background:C.bg,animation:"fadeIn .2s"}},
    ...
    e("div",{style:{position:"sticky",top:62,zIndex:10,background:"rgba(6,10,20,.94)",backdropFilter:"blur(12px)",borderBottom:"1px solid "+C.bd,padding:"10px 14px"}},
      e("div",{style:{display:"flex",alignItems:"center",gap:10,marginBottom:10}},
        e("div",{style:{flex:1,minWidth:0}},
          e("div",{style:{fontSize:9.5,fontWeight:800,color:c,letterSpacing:1.8,textTransform:"uppercase",marginBottom:1,opacity:.85}},p.kicker||"Enfermedad"),
          e("div",{style:{fontSize:15,fontWeight:800,color:"#fff",lineHeight:1.2,fontFamily:"Playfair Display"}},p.name)
        )
      ),

...
function SV_sm2(prog,rating){
  ...
  if(rating===0){
    ...
    newEf=ef;
    if(reps===0){ newIntv=1; }
    else if(reps===1){ newIntv=6; }
    else { newIntv=Math.round(intv*ef); }
    newReps=reps+1;
    nextReview=new Date(Date.now()+newIntv*86400000).toISOString();
  } else {
    newEf=ef+0.15;
    if(reps===0){ newIntv=1; }
    else if(reps===1){ newIntv=8; }
    else { newIntv=Math.round(intv*newEf*1.3); }
    newReps=reps+1;
    nextReview=new Date(Date.now()+newIntv*86400000).toISOString();
  }
...
function StudyView(props){
  ...
  s=useState([]);       var SV_results=s[0],       SV_setResults=s[1];
  ...
  useEffect(function(){
    if(!SV_styleInjected){
      ...
      st.textContent=
        "@keyframes SV_slideOut{0%{transform:translateX(0);opacity:1}100%{transform:translateX(-50px);opacity:0}}" +
        "@keyframes slideUp{0%{transform:translateY(16px);opacity:0}100%{transform:translateY(0);opacity:1}}" +
        ".sv-out{animation:SV_slideOut .22s ease-in both}" +
        ".sv-card{animation:slideUp .25s ease-out both}" +
        ".sv-card:hover{box-shadow:0 4px 20px rgba(167,139,250,.15)}";
      ...
      .then(function(deckRes){
        ...
        Promise.all([cardsPromise,progPromise]).then(function(results){
          ...
          if(props.deck&&props.deck.id){
            ...
            for(var di=0;di<decksData.length;di++){
              initSel[decksData[di].id]=true;
            }
          }
  ...
  var selDeckSet={};
  for(var k in SV_selIds){
    if(SV_selIds.hasOwnProperty(k)) selDeckSet[k]=true;
  }
  ...
  function SV_toggleDeck(id){
    var next={};
    for(var dk in SV_selIds){
      if(SV_selIds.hasOwnProperty(dk)) next[dk]=SV_selIds[dk];
    }
  ...
  function SV_toggleTag(t){
    var idx=-1;
    for(var i=0;i<SV_selTags.length;i++){
      if(SV_selTags[i]===t){ idx=i; break; }
    }
    var next=[];
  ...
  function SV_rate(rating){
    if(SV_rated) return;
    SV_setRated(true);
    var card=SV_session[SV_idx];
    var prog=SV_progress[card.id];
    var newProg=SV_sm2(prog,rating);
    var nextProg={};
    for(var pk in SV_progress){
      if(SV_progress.hasOwnProperty(pk)) nextProg[pk]=SV_progress[pk];
    }
    nextProg[card.id]=newProg;
    SV_setProgress(nextProg);
    var newResults=[];
    for(var ri=0;ri<SV_results.length;ri++){ newResults.push(SV_results[ri]); }
    newResults.push({card:card,rating:rating});
    SV_setResults(newResults);
    if(window.ECEPT_SUPABASE&&user){
      window.ECEPT_SUPABASE.from("flashcard_progress").upsert({
        user_id:user.id,
      ...
      },{onConflict:"user_id,flashcard_id"}).catch(function(){});
    }
    SV_setSliding(true);
  ...
  if(SV_phase==="setup"){
    var modeOpts=[
      {id:"today",icon:"📅",label:"Pendientes hoy",desc:"Tarjetas vencidas o nuevas"},
      {id:"all",icon:"📚",label:"Todas",desc:"Cualquier tarjeta del mazo"},
      {id:"new",icon:"✨",label:"Solo nuevas",desc:"Nunca estudiadas"},
      {id:"review",icon:"🔁",label:"Solo repaso",desc:"Ya vistas al menos una vez"}
    ];
    ...
    return e("div",{style:{maxWidth:"760px",margin:"0 auto",padding:"20px 16px 80px"}},
      ...
      e("div",{style:{display:"flex",alignItems:"center",gap:"12px",marginBottom:"24px"}},
        ...
        e("h1",{style:{
          fontSize:"22px",fontWeight:700,color:C.tx,margin:0,flex:1
        }},"Modo estudio"),
      ...
      // Loading spinner
      SV_loadingSetup ? e("div",{style:{
        textAlign:"center",padding:"60px 20px",color:C.mt,fontSize:"15px"
      }},
        ...
        allTags.length>0 ? e("div",{style:{marginBottom:"24px"}},
          ...
          SV_tagExp ? e("div",{style:{display:"flex",flexWrap:"wrap",gap:"6px"}},
            ...
            allTags.map(function(t){
              ...
              return e("button",{
                key:t,
                onClick:function(){ SV_toggleTag(t); },
                style:{
                  padding:"5px 12px",borderRadius:"16px",fontSize:"12px",cursor:"pointer",
        ...
        e("div",{style:{marginBottom:"24px"}},
          ...
          e("div",{style:{display:"flex",gap:"8px",flexWrap:"wrap"}},
            modeOpts.map(function(mo){
              ...
              return e("button",{
                key:mo.id,
                onClick:function(){ SV_setMode(mo.id); },
                style:{
                  flex:"1 1 140px",minWidth:"130px",
                  display:"flex",flexDirection:"column",alignItems:"flex-start",
                  gap:"4px",padding:"14px 14px",borderRadius:"12px",cursor:"pointer",
                  border:"1px solid "+(sel?"#a78bfa":C.bd),
                  background:sel?"rgba(167,139,250,.1)":C.cd,
                  textAlign:"left"
                }
              },
        ...
        e("div",{style:{marginBottom:"28px"}},
          e("div",{style:{
            ...
            e("span",{style:{fontSize:"12px",color:C.mt}},
          ...
          e("div",{style:{display:"flex",alignItems:"center",gap:"14px"}},
            ...
            e("span",{style:{
              minWidth:"44px",textAlign:"center",fontSize:"18px",
  ...
  if(SV_phase==="study"){
    ...
    var progressPct=SV_session.length>0
      ? Math.round((SV_idx/SV_session.length)*100)
      : 0;

    return e("div",{style:{maxWidth:"640px",margin:"0 auto",padding:"20px 16px 80px"}},
      ...
      e("div",{style:{marginBottom:"20px"}},
        e("div",{style:{
          ...
          e("button",{
            ...
            style:{
              background:"none",border:"none",color:C.mt,
              fontSize:"14px",cursor:"pointer",padding:"6px 10px",borderRadius:"8px"
            }
          },"← Salir"),
          e("span",{style:{fontSize:"14px",fontWeight:600,color:C.mt}},
        ...
        // Progress bar
        e("div",{style:{
          height:"4px",borderRadius:"2px",background:C.bd,overflow:"hidden"
        }},
          e("div",{style:{
      ...
      e("div",{
        key:"card-"+SV_idx,
        className:(SV_sliding?"sv-out ":"")+"sv-card",
        onClick:function(){
          if(!SV_flipped&&!SV_rated) SV_setFlipped(true);
        },
        style:{
          minHeight:"220px",borderRadius:"16px",padding:"28px 24px",
          background:"linear-gradient(135deg,"+C.cd+","+deckCol+"12)",
          border:"1px solid "+deckCol+"30",
          borderBottom:"4px solid "+deckCol,
          cursor:SV_flipped?"default":"pointer",
        ...
        e("div",{style:{
          ...
          color:deckCol,opacity:0.7,textTransform:"uppercase"
        }},
        ...
        // Front text
        e("div",{style:{
          fontSize:"17px",lineHeight:1.6,color:C.tx,
          textAlign:"center",marginBottom:SV_flipped&&!isCloze?"0":"0"
        }},
          isCloze
            ? SV_clozeEl(currentCard.front,!SV_flipped)
            : e("span",null,currentCard.front||"")
        ),

        // Separator + back (if flipped, basic only)
        SV_flipped&&!isCloze ? e(F,null,
          e("div",{style:{
            height:"1px",background:deckCol+"30",
            margin:"18px 0"
          }}),
          e("div",{style:{
            fontSize:"16px",lineHeight:1.6,color:"#a5f3c4",
            textAlign:"center"
          }},
            currentCard.back||""
          )
        ...
        // Tap hint
        !SV_flipped ? e("div",{style:{
          position:"absolute",bottom:"12px",left:"0",right:"0",
          textAlign:"center",fontSize:"12px",color:C.dm
        }},"Toca para revelar") : null
      ),

      // "Mostrar respuesta" button if not flipped
      !SV_flipped ? e("button",{
        ...
        ratingBtns.map(function(rb){
          return e("button",{
            ...
            style:{
              ...
              cursor:SV_rated?"not-allowed":"pointer",
              opacity:SV_rated?0.5:1,
              transition:"all .15s"
            }
          },
            e("span",{style:{fontSize:"13px",fontWeight:700,color:rb.color}},rb.label),
            e("span",{style:{fontSize:"11px",color:C.dm}},
              SV_intervalLabel(currentProg,rb.rating)
            )
          );
        })
      ) : null
  ...
  if(SV_phase==="summary"){
    ...
    for(var ri2=0;ri2<SV_results.length;ri2++){
      var rr=SV_results[ri2].rating;
      if(ratingCounts.hasOwnProperty(rr)) ratingCounts[rr]++;
    }
    var total=SV_results.length;
    ...
    return e("div",{style:{
      ...
      e("h1",{style:{
        fontSize:"26px",fontWeight:700,color:C.tx,marginBottom:"8px"
      }},"¡Sesión completa!"),
      e("p",{style:{color:C.mt,fontSize:"14px",marginBottom:"28px"}},
        total+" tarjeta"+(total===1?"":"s")+" · "+timeStr
      ),
        ...
        ratingRows.map(function(row){
          ...
          return e("div",{key:row.rating,style:{marginBottom:"12px"}},
            e("div",{style:{
              ...
              e("span",{style:{fontSize:"13px",color:C.mt}},
                cnt+" ("+pct+"%)"
              )
            ),
            e("div",{style:{
              height:"6px",borderRadius:"3px",background:C.bd,overflow:"hidden"
            }},
              e("div",{style:{
